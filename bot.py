...
def should_ai_chat(message):
    ...
    if (
        enabled
        and "메인채팅" in message.channel.name
    ):
        return True
...
async def on_message(message):
    ...
    # 미성년자는 AI 응답 안 함
    if (
        mode in ["adult", "adult_19"]
        and not is_adult(message.author)
    ):
