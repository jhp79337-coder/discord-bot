import discord
from discord.ext import commands, tasks
from datetime import datetime, timedelta, timezone
from pathlib import Path
import json
import os
import re
from dotenv import load_dotenv

# ============================================================
# 새벽에너를기다리는중 자동관리봇
#
# 기능
# 1. 신규 멤버 기록
# 2. 자기소개 채널에 자기소개 작성 시 완료 처리
# 3. 채팅 활동 기록
# 4. 음성채널 활동 기록
# 5. 가입 후 3일이 지나도 자기소개 X + 활동 X면 자동 추방
# 6. 관리자 / 봇은 자동 추방 제외
# 7. !상태 명령어로 상태 확인
# 8. !검사 명령어로 추방 대상 확인
# ============================================================

# ------------------------------------------------------------
# 설정
# ------------------------------------------------------------

# .env 파일에서 디스코드 봇 토큰 읽기
load_dotenv()

TOKEN = os.getenv("DISCORD_TOKEN")

# 자기소개 채널 이름에 이 단어가 들어가면 자동으로 찾음
INTRO_KEYWORD = "자기소개"

# 추방 로그 채널 이름
LOG_CHANNEL_NAME = "추방-로그"

# 가입 후 유예 기간
GRACE_DAYS = 3

# 몇 분마다 자동 검사할지
CHECK_MINUTES = 30

# 멤버 데이터 저장 파일
DATA_FILE = Path(__file__).parent / "members.json"


# ------------------------------------------------------------
# 디스코드 설정
# ------------------------------------------------------------

intents = discord.Intents.default()

# 멤버 정보
intents.members = True

# 채팅 내용
intents.message_content = True

# 음성채널 상태
intents.voice_states = True

bot = commands.Bot(
    command_prefix="!",
    intents=intents
)


# ------------------------------------------------------------
# 시간
# ------------------------------------------------------------

def utcnow():
    """현재 UTC 시간"""
    return datetime.now(timezone.utc)


# ------------------------------------------------------------
# 데이터 관리
# ------------------------------------------------------------

def load_data():
    """members.json 불러오기"""

    if not DATA_FILE.exists():
        return {}

    try:
        return json.loads(
            DATA_FILE.read_text(encoding="utf-8")
        )

    except Exception as e:
        print(f"[데이터 불러오기 오류] {e}")
        return {}


def save_data():
    """members.json 저장"""

    try:
        DATA_FILE.write_text(
            json.dumps(
                members_data,
                ensure_ascii=False,
                indent=2
            ),
            encoding="utf-8"
        )

    except Exception as e:
        print(f"[데이터 저장 오류] {e}")


members_data = load_data()


# ------------------------------------------------------------
# 멤버 기록 생성 / 확인
# ------------------------------------------------------------

def ensure_member(member: discord.Member):
    """멤버 기록이 없으면 새로 생성"""

    key = str(member.id)

    if key not in members_data:

        members_data[key] = {
            "joined": utcnow().isoformat(),
            "last_activity": utcnow().isoformat(),
            "intro": False
        }

        save_data()

    return members_data[key]


# ------------------------------------------------------------
# 자동 추방 제외 여부
# ------------------------------------------------------------

def is_exempt(member: discord.Member):
    """자동 추방에서 제외할 사람"""

    # 봇 제외
    if member.bot:
        return True

    # 관리자 권한 보유자 제외
    if member.guild_permissions.administrator:
        return True

    return False


# ------------------------------------------------------------
# 자기소개 채널 찾기
# ------------------------------------------------------------

def find_intro_channel(guild: discord.Guild):

    # 예:
    # 자기소개
    # 👋・자기소개
    # 📢-자기소개
    # 자기소개-채널
    #
    # 전부 찾을 수 있음

    for channel in guild.text_channels:

        if INTRO_KEYWORD in channel.name:
            return channel

    return None


# ------------------------------------------------------------
# 로그 채널 찾기
# ------------------------------------------------------------

def find_log_channel(guild: discord.Guild):

    return discord.utils.get(
        guild.text_channels,
        name=LOG_CHANNEL_NAME
    )


# ------------------------------------------------------------
# 봇 로그인
# ------------------------------------------------------------

@bot.event
async def on_ready():

    print("=" * 60)
    print(f"로그인 완료: {bot.user}")
    print(f"봇 ID: {bot.user.id}")
    print(f"연결된 서버: {len(bot.guilds)}개")
    print("=" * 60)

    # 자동 검사 중복 실행 방지
    if not check_members.is_running():
        check_members.start()


# ------------------------------------------------------------
# 신규 멤버 입장
# ------------------------------------------------------------

@bot.event
async def on_member_join(member: discord.Member):

    # 봇은 기록하지 않음
    if member.bot:
        return

    members_data[str(member.id)] = {

        "joined": utcnow().isoformat(),

        "last_activity": utcnow().isoformat(),

        "intro": False

    }

    save_data()

    print(f"[입장] {member} ({member.id})")

    # 자기소개 채널 자동 찾기
    intro_channel = find_intro_channel(member.guild)

    if intro_channel:

        try:

            await intro_channel.send(
                f"{member.mention} 환영합니다! 👋\n\n"
                f"**3일 이내에 자기소개를 작성해주세요.**\n"
                f"채팅이나 음성채널 활동도 자유롭게 해주세요.\n\n"
                f"⚠️ **자기소개도 없고 3일간 활동도 없으면 자동 퇴장 처리됩니다.**"
            )

        except discord.Forbidden:

            print(
                f"[권한 오류] {intro_channel.name} 채널에 "
                f"메시지를 보낼 수 없습니다."
            )


# ------------------------------------------------------------
# 채팅 활동
# ------------------------------------------------------------

@bot.event
async def on_message(message: discord.Message):

    # 봇 메시지는 무시
    if message.author.bot:
        return

    # 서버 멤버인지 확인
    if isinstance(message.author, discord.Member):

        member = message.author

        data = ensure_member(member)

        # --------------------------------------------
        # 일반 채팅 활동 기록
        # --------------------------------------------

        data["last_activity"] = utcnow().isoformat()

        # --------------------------------------------
        # 자기소개 채널 확인
        # --------------------------------------------

        if isinstance(message.channel, discord.TextChannel):

            if INTRO_KEYWORD in message.channel.name:

                intro_text = message.content.strip()

                # ----------------------------------------
                # 자기소개 형식
                #
                # 예:
                # 04 남
                # 04 여
                # 99 남
                # 01 여
                #
                # 띄어쓰기 없어도 가능
                # 04남
                # 04여
                # ----------------------------------------

                if re.fullmatch(
                    r"\d{2}\s*(남|여)",
                    intro_text
                ):

                    data["intro"] = True

                    save_data()

                    print(
                        f"[자기소개 완료] "
                        f"{member} -> {intro_text}"
                    )

                    # 완료 표시
                    try:

                        await message.add_reaction("✅")

                    except discord.HTTPException:

                        pass

                else:

                    # 자기소개 채널에서 다른 글을 쓴 경우에도
                    # 활동 자체는 기록됨
                    save_data()

        else:

            save_data()

    # 명령어 처리
    await bot.process_commands(message)


# ------------------------------------------------------------
# 음성채널 활동
# ------------------------------------------------------------

@bot.event
async def on_voice_state_update(
    member: discord.Member,
    before: discord.VoiceState,
    after: discord.VoiceState
):

    # 봇 제외
    if member.bot:
        return

    # 음성채널에 새로 들어간 경우
    if before.channel is None and after.channel is not None:

        data = ensure_member(member)

        data["last_activity"] = utcnow().isoformat()

        save_data()

        print(
            f"[음성 활동] "
            f"{member} -> {after.channel.name}"
        )


# ------------------------------------------------------------
# 자동 추방 검사
# ------------------------------------------------------------

@tasks.loop(minutes=CHECK_MINUTES)
async def check_members():

    current = utcnow()

    print(
        f"[자동 검사] "
        f"{current.strftime('%Y-%m-%d %H:%M:%S')}"
    )

    for guild in bot.guilds:

        # 서버 멤버 전체 검사
        for member in guild.members:

            # 봇 / 관리자 제외
            if is_exempt(member):
                continue

            # 데이터가 없는 사람
            data = members_data.get(str(member.id))

            if not data:
                continue

            try:

                joined = datetime.fromisoformat(
                    data["joined"]
                )

                last_activity = datetime.fromisoformat(
                    data["last_activity"]
                )

            except (
                KeyError,
                ValueError,
                TypeError
            ):

                continue

            # --------------------------------------------
            # 가입 후 3일 미만이면 제외
            # --------------------------------------------

            if current - joined < timedelta(
                days=GRACE_DAYS
            ):
                continue

            # --------------------------------------------
            # 자기소개 작성했다면 제외
            # --------------------------------------------

            if data.get("intro", False):
                continue

            # --------------------------------------------
            # 최근 3일 이내 활동했다면 제외
            # --------------------------------------------

            if current - last_activity < timedelta(
                days=GRACE_DAYS
            ):
                continue

            # --------------------------------------------
            # 여기까지 왔다면 자동 추방 대상
            # --------------------------------------------

            try:

                log_channel = find_log_channel(guild)

                # ----------------------------------------
                # 추방 로그
                # ----------------------------------------

                if log_channel:

                    embed = discord.Embed(

                        title="🚪 자동 추방",

                        description=(
                            f"{member.mention} 님이 "
                            f"**자기소개 미작성 + 장기 미활동**으로 "
                            f"자동 추방되었습니다."
                        ),

                        timestamp=current
                    )

                    embed.add_field(

                        name="사용자",

                        value=(
                            f"{member} "
                            f"({member.id})"
                        ),

                        inline=False
                    )

                    embed.add_field(

                        name="사유",

                        value=(
                            "가입 후 3일 경과 / "
                            "자기소개 없음 / "
                            "3일간 활동 없음"
                        ),

                        inline=False
                    )

                    await log_channel.send(
                        embed=embed
                    )

                # ----------------------------------------
                # 추방
                # ----------------------------------------

                await member.kick(
                    reason=(
                        "자기소개 미작성 + "
                        "3일간 활동 없음"
                    )
                )

                print(
                    f"[자동 추방] {member}"
                )

                # ----------------------------------------
                # 데이터 삭제
                # ----------------------------------------

                members_data.pop(
                    str(member.id),
                    None
                )

                save_data()

            except discord.Forbidden:

                print(
                    f"[권한 오류] {member} 추방 실패"
                )

                print(
                    "봇 역할을 대상 멤버보다 위로 올리고 "
                    "Kick Members 권한을 확인하세요."
                )

            except Exception as e:

                print(
                    f"[추방 오류] "
                    f"{member}: {e}"
                )


# ------------------------------------------------------------
# 자동 검사 시작 전 대기
# ------------------------------------------------------------

@check_members.before_loop
async def before_check_members():

    await bot.wait_until_ready()


# ------------------------------------------------------------
# !상태
#
# 사용:
# !상태
#
# 또는
#
# !상태 @사람
# ------------------------------------------------------------

@bot.command(name="상태")
@commands.has_permissions(administrator=True)
async def status(
    ctx,
    member: discord.Member = None
):

    # 멤버를 지정하지 않으면 자기 자신
    member = member or ctx.author

    data = members_data.get(
        str(member.id)
    )

    if not data:

        await ctx.send(
            "📌 이 사용자의 기록이 없습니다."
        )

        return

    # 자기소개 상태
    if data.get("intro", False):

        intro = "✅ 작성 완료"

    else:

        intro = "❌ 미작성"

    # 최근 활동 시간
    try:

        last = datetime.fromisoformat(
            data["last_activity"]
        )

        last_activity_text = (
            f"<t:{int(last.timestamp())}:R>"
        )

    except Exception:

        last_activity_text = "알 수 없음"

    await ctx.send(

        f"**{member.display_name} 상태**\n"
        f"자기소개: {intro}\n"
        f"최근 활동: {last_activity_text}"

    )


# ------------------------------------------------------------
# !검사
#
# 현재 자동 추방 조건에 해당하는 사람 수 확인
# ------------------------------------------------------------

@bot.command(name="검사")
@commands.has_permissions(administrator=True)
async def manual_check(ctx):

    count = 0

    current = utcnow()

    for guild in bot.guilds:

        for member in guild.members:

            if is_exempt(member):
                continue

            data = members_data.get(
                str(member.id)
            )

            if not data:
                continue

            try:

                joined = datetime.fromisoformat(
                    data["joined"]
                )

                last_activity = datetime.fromisoformat(
                    data["last_activity"]
                )

            except Exception:

                continue

            # 가입 후 3일 미만
            if current - joined < timedelta(
                days=GRACE_DAYS
            ):

                continue

            # 자기소개 완료
            if data.get("intro", False):

                continue

            # 최근 활동 있음
            if current - last_activity < timedelta(
                days=GRACE_DAYS
            ):

                continue

            count += 1

    await ctx.send(
        f"🔍 현재 자동 추방 조건에 해당하는 멤버: "
        f"**{count}명**"
    )


# ------------------------------------------------------------
# !자기소개초기화
#
# 사용:
# !자기소개초기화 @사람
# ------------------------------------------------------------

@bot.command(name="자기소개초기화")
@commands.has_permissions(administrator=True)
async def reset_intro(
    ctx,
    member: discord.Member
):

    data = ensure_member(member)

    data["intro"] = False

    save_data()

    await ctx.send(
        f"🔄 {member.mention}님의 "
        f"자기소개 상태를 초기화했습니다."
    )


# ------------------------------------------------------------
# 명령어 오류 처리
# ------------------------------------------------------------

@bot.event
async def on_command_error(
    ctx,
    error
):

    # 명령어 없음
    if isinstance(
        error,
        commands.CommandNotFound
    ):
        return

    # 관리자 권한 없음
    if isinstance(
        error,
        commands.MissingPermissions
    ):

        await ctx.send(
            "❌ 이 명령어는 관리자만 사용할 수 있습니다."
        )

        return

    # 멤버 지정 안 함
    if isinstance(
        error,
        commands.MissingRequiredArgument
    ):

        await ctx.send(
            "❌ 사용법을 확인해주세요."
        )

        return

    print(
        f"[명령어 오류] {error}"
    )


# ------------------------------------------------------------
# 토큰 확인 후 봇 실행
# ------------------------------------------------------------

if not TOKEN:

    print(
        "❌ DISCORD_TOKEN을 설정해주세요."
    )

    print(
        "bot.py와 같은 폴더에 있는 .env 파일을 "
        "확인해주세요."
    )

else:

    print("✅ 디스코드 토큰 확인 완료")
    print("🤖 봇을 시작합니다...")

    bot.run(TOKEN)
